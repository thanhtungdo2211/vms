...
# Scenario:
#   1. Add cam1-cam8 to branch recognition
#   2. Remove cam6, cam7, cam8
#   3. Re-add cam6, cam7, cam8
#   4. Stream cam1 recognition to RTSP server

CAMERA_URI="rtsp://192.168.6.14:8554/testface"
STREAM_SERVER="152.42.221.89"
STREAM_PORT="8554"
STREAM_NAME_CAM1="cam1_recognition"
STREAM_NAME_CAM2="cam2_recognition"
...
# Step 1: Add cam1-cam8 to branch recognition
echo -e "\n[1] Adding cam1-cam8 to branch recognition..."
for i in {1..8}; do
  echo "Adding cam${i}..."
  response=$(curl -s -X POST http://localhost:8083/api/cameras \
    -H "Content-Type: application/json" \
    -d '{"camera_id":"cam'$i'","uri":"'$CAMERA_URI'","branch":"recognition"}')
...
echo -e "\n[3] Re-adding cam6, cam7, cam8 to branch recognition..."
for i in {6..8}; do
...
# Step 4: Stream cam1 recognition
echo -e "\n[4] Starting stream cam1_recognition to $STREAM_URI_CAM1..."
response=$(curl -s -X POST http://localhost:8083/api/cameras/cam1/branches/recognition/stream/start \
  -H "Content-Type: application/json" \
...
# Step 5: Stream cam2 recognition
echo -e "\n[5] Starting stream cam2_recognition to $STREAM_URI_CAM2..."
response=$(curl -s -X POST http://localhost:8083/api/cameras/cam2/branches/recognition/stream/start \
  -H "Content-Type: application/json" \
...
# Verify stream cam1
echo -e "\n[6] Verifying stream cam1_recognition..."
...
# Verify stream cam2
echo -e "\n[7] Verifying stream cam2_recognition..."
if ffprobe -v quiet -rtsp_transport tcp -stimeout 8000000 \
